...
import { XpenseModel, useXpenseRecords } from "../../contexts/xpense-record-context";
import { useTable, Column, CellProps } from "react-table";

interface EditableCellProps extends CellProps<XpenseModel> {
    updateRecord: (rowIndex: number, columnId: string, value: string) => void;
    editable: boolean;  // this is becz if the specific cell is editable or not
}
...
export const XpenseXplorerList = () => {
  const { records, updateRecord, deleteRecord } = useXpenseRecords();
  ...
  const columns: Array<Column<XpenseModel>> = useMemo(()=>[
    {
        Header: "Description",
  ...
  return (
    <div className="table-container">
      <table {...getTableProps()} className="table">
        <thead>
          {headerGroups.map((headgp) => (
            <tr {...headgp.getHeaderGroupProps()}>
              {headgp.headers.map((column) => (
